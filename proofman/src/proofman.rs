...
use proofman_common::TimedBufferPool;
...
use proofman_starks_lib_c::{
    ...
    get_last_proof_timing_c, PROOF_TIMING_LAUNCH_PROLOGUE, PROOF_TIMING_RELOAD_FIXED_POLS,
};
...
const PROOF_RECORDS_TAG: usize = 999995;
...
pub struct ProofMan<F: PrimeField64> {
    ...
    proof_record_sends: Mutex<Vec<std::thread::JoinHandle<()>>>,
...
impl<F: PrimeField64> Drop for ProofMan<F> {
    fn drop(&mut self) {
        ...
        for send in self.proof_record_sends.get_mut().unwrap().drain(..) {
            let _ = send.join();
        }
...
impl<F: PrimeField64> ProofMan<F> {
    ...
    pub fn reset(&self) -> ProofmanResult<()> {
        ...
        self.proof_recorder.reset();
        while self.mpi_ctx.check_incoming_proofs(PROOF_RECORDS_TAG).is_some() {}
    ...
    GoldilocksQuinticExtension: ExtensionField<F>,
{
    ...
    pub fn take_proof_records(&self) -> Vec<ProofRecord> {
        self.proof_recorder.take()
    }

    fn gather_proof_records(&self) {
        if self.mpi_ctx.rank != 0 {
            let (mpi_ctx, words) = (self.mpi_ctx.clone(), self.proof_recorder.export());
            let mut sends = self.proof_record_sends.lock().unwrap();
            sends.retain(|send| !send.is_finished());
            sends.push(std::thread::spawn(move || mpi_ctx.send_proof_to_rank(&words, PROOF_RECORDS_TAG, 0)));
            return;
        }
        let mut pending = self.mpi_ctx.n_processes - 1;
        while pending > 0 && !self.cancellation_info.read_recover().token.is_cancelled() {
            match self.mpi_ctx.check_incoming_proofs(PROOF_RECORDS_TAG) {
                Some(words) => pending -= self.proof_recorder.import(&words) as i32,
    ...
    pub fn new(proving_key_path: PathBuf, options: ProofmanOptions) -> ProofmanResult<Self> {
        ...
        Ok(Self {
            ...
            proof_recorder: Arc::new(ProofRecorder::new()),
            proof_record_sends: Mutex::new(Vec::new()),
            computing: Mutex::new(()),
        })
    }
    ...
    fn _generate_proof(
        ...
        let all_partial_contributions_u64 = if phase == ProvePhase::Contributions || phase == ProvePhase::Full {
            ...
            for instance_id in my_instances_tables.iter() {
                ...
                let span = witness_build_span(&buffer_pool, prepare_start, prepare_end);
                self.proof_recorder.record_build(id, RECORD_KIND_WITNESS, airgroup_id, air_id, span);
            }
            ...
            self.proof_recorder.next_job();
            self.gather_proof_records();
        ...
        let prefetch_dequeue_ahead = self.pctx.gpu
            && self.n_streams_non_recursive == 1
    ...
    fn outer_aggregations(&self) -> CompletionOwner {
            ...
            let handle_recursive = std::thread::spawn(move || {
                ...
                while let Ok(msg) = recursive_rx_clone.recv() {
                    ...
                    if recursive2_airgroup_proofs.len() >= arity {
                        ...
                        let witness = match w {
                            Ok((witness, witness_start)) => {
                                proof_recorder_clone.stash_span(witness.trace.as_ptr() as usize, witness_start);
                                witness
                            }
                            Err(e) => {
        ...
        for _ in 0..self.n_streams {
            ...
            let handle = std::thread::spawn(move || loop {
                ...
                witness.global_idx = Some(id);
                proof_recorder_clone.record_stashed_span(
                    witness.trace.as_ptr() as usize,
                    id as u64,
                    RECORD_KIND_AGGREGATION_WITNESS,
                    witness.airgroup_id,
                    witness.air_id,
        ...
        let witness_handler = if !minimal_memory && (self.pctx.gpu || stats) {
            ...
            Some(std::thread::spawn(move || loop {
                ...
                *in_flight.lock().unwrap().entry((airgroup_id, air_id)).or_insert(0) += 1;
                ...
                let recorder = proof_recorder_clone.clone();
                let handle = std::thread::spawn(move || {
                    ...
                    let span = witness_build_span(&buffer_pool, compute_start, compute_start);
                    drop(tokens);
                    ...
                    if first_admission {
                        recorder.record_build(instance_id as u64, RECORD_KIND_WITNESS, airgroup_id, air_id, span);
                    }
                    if stats {
    ...
    fn calculate_witness(
        ...
        if !minimal_memory && (self.pctx.gpu || stats) {
            ...
            for &instance_id in instances.iter() {
                ...
                let witness_done_clone = witness_done.clone();
                let cancellation_info_clone = self.cancellation_info.clone();
                let recorder = self.proof_recorder.clone();
                let handle = std::thread::spawn(move || {
                    timer_start_debug!(GENERATING_WC, "GENERATING_WC_{} [{}:{}]", instance_id, airgroup_id, air_id);
                    let buffer_pool = TimedBufferPool::new(memory_handler_clone.as_ref());
                    let prepare_start = std::time::Instant::now();
                    ...
                    witness_done_clone.increment();
                    recorder.record_build(instance_id as u64, RECORD_KIND_RECOMPUTE, airgroup_id, air_id, span);
                    if stats {
                        let (is_shared_buffer, witness_buffer) = pctx_clone.free_instance_traces(instance_id);
                        if is_shared_buffer {
                            if let Err(e) = memory_handler_clone.release_buffer(witness_buffer) {
    ...
    fn get_contribution_air(
        ...
        let stream_id = if slot_committed {
            recorder.record_completion(instance_id as u64, RECORD_KIND_COMMIT, get_last_proof_timing_c(u64::MAX));
            u64::MAX
        } else {
            commit_witness_c(
                p_setup,
