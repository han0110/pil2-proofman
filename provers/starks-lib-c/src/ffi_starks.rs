...
// Read by `on_proof_done` (C++/CUDA harvest threads), written by register/clear.
// A late harvest can race a `clear`; the RwLock keeps read-vs-drop
...
extern "C" fn on_proof_done(instance_id: u64, proof_type: *const c_char, timing: *const ProofTiming) {
    ...
    let breakdown_ms =
        unsafe { timing.as_ref() }.map_or([0; PROOF_TIMING_SECTIONS], |t| t.sections.map(|ms| ms.round() as u32));
...
pub fn get_last_proof_timing_c(stream_id: u64) -> [u32; PROOF_TIMING_SECTIONS] {
    ...
    timing.sections.map(|ms| ms.round() as u32)
}
...
pub fn clear_proof_done_callback_c() {
    ...
    // mid-send. A late harvest after clearing just observes `None`.
